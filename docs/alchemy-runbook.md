...
| Health `UP` with `lastDataError` `No Alchemy network is mapped for chain ...` or `No enabled asset config for ...`; the address's run `FAILED` | one address's chain lost its network mapping, or its asset config was disabled, after registration (terminal for that address only) | restore the mapping or the asset config and re-enqueue, or disable the address |
| Run `FAILED`, `last_error` `Alchemy provider cursor is not valid JSON, so another provider wrote it` or `belongs to provider '...'` | the address was synced by the HTTP bridge or the demo simulator before the switch; its cursor means nothing here | clear it and re-enqueue: `DELETE FROM sync_cursors WHERE watched_address_id = '<id>';` The next sync starts as `start-mode` says, and blocks between the old provider's last event and that start are not read unless `configured-block` covers them |
...
Rules that never change: do not persist a `pageKey` or a mid-block position in `provider_cursor`, do not clear an Alchemy cursor to skip a block (only a cursor another provider wrote is cleared), and do not put the key into a URL unless `path` auth mode is unavoidable.
