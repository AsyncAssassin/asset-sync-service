...
A configuration gap of one address, such as its chain without a network mapping, its asset without an enabled config, or a block with more events than a page, is an `AddressConfigurationException`: terminal for that address's runs and shown as the provider's `lastDataError`, while health stays `UP`. Registration refuses a chain without a mapping in the first place (`ChainProviderPort.supportsChain`), so only an address registered before the switch or a mapping removed later gets there.
