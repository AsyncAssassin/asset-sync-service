...
- `events` (required): objects with `txHash`, `eventIndex`, `address`, `asset`, `amount` (a JSON number or decimal string), `blockHeight`, `confirmations` (these three JSON integers: a fraction fails the page), `direction` (`INBOUND` or `OUTBOUND`), and `status` (`SEEN`, `CONFIRMED`, or `REVERTED`), in non-decreasing `(blockHeight, eventIndex, txHash)` order and at most `limit` of them. A page carries one row per `(txHash, eventIndex)`: an exact repeat is harmless, while a repeat with another direction or amount fails the page before anything is written, so a transfer of the address to itself is left out, as the Alchemy adapter does.
