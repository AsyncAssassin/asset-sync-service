...
- Rolling back to 0.4.1 after `017` and `018`: 0.4.1 cannot read `PROVIDER_BUSY` or `ADDRESS_RETRY` and stops claiming runs while one exists, so first run `UPDATE sync_runs SET last_requeue_reason = 'CONTINUATION' WHERE last_requeue_reason IN ('PROVIDER_BUSY', 'ADDRESS_RETRY');`. 0.4.1 also ignores the retry list of an account pass, so an account run it resumes can succeed without the addresses that waited for a retry: sync such accounts again after the rollback. Later versions read a reason they do not know as none.
...
- Healthy provider pagination continuations increment `continuation_count`, not `failure_attempts`. So do a busy cursor lease (`LEASE_BUSY`) and a full provider pool (`PROVIDER_BUSY`), both retried after `asset-sync.sync.pagination.cursor-lease-retry-delay`, and an account pass with addresses waiting to retry (`ADDRESS_RETRY`), due when the first of them is. All are bounded by `max-continuations-per-run`.
