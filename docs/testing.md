...
| Profile guards | `prod` combined with `demo` or `local` stops before any bean is created and names the active profiles, so the demo users are never seeded; a protected profile other than `prod`, such as a custom `staging`, refuses a database that holds the demo users as well |
