...
| Bridge credentials | an unreachable bridge whose `base-url` carries secrets in the userinfo, the path, and the query: `lastError` for a `READ` caller, health, and the logs name the failure kind and no secret; the kind is found anywhere in the cause chain; a read timeout is reported as a timeout |
