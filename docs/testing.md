...
| HTTP bridge contract | a cursor with `+ / = & % { }` reaches the stub bridge unchanged; the credential header is sent and never logged; a redirect is not followed and fails as configuration with health `DOWN`, as do `401` and `403`, while `404` and a body that is not JSON text stay the address's error; an idle final page without a cursor at an unchanged height succeeds and keeps the cursor; a page repeating an event with another direction is refused before anything is written; a full provider pool requeues without spending the retry budget |
| Slow providers | a bridge or Alchemy body that trickles past a cancelled fetch gives the provider thread back; a body over the byte limit and an error status with an endless body fail at once, their connections closed rather than drained |
...
| Account traversal | busy early address is skipped while later addresses are processed; a pass larger than one claim resumes from its keyset and completes; an address registered between claims joins the pass; an address with pages left is drained before the scan moves on; a busy address is revisited after the scan; a terminally failing address is reported in `last_error` while the others sync, and disabling it lets the account sync pass; an address that keeps timing out is recorded after the attempts of a run while the others sync, with no `failure_attempts` spent; three addresses failing in a row fail the claim as an outage that costs them nothing, and the retried run takes its due retries first and resumes its pass; two stubborn addresses are each recorded, not taken for an outage; a throttled fetch fails the claim; pages committed before a failure count as progress and break a streak; a full retry list holds the scan until the retries drain it; a claim interrupted by shutdown, even one that reached only the provider thread, saves the pass; the pass checkpoint with every list full fits `run_checkpoint`; a requeue reason from a later version does not stop a run from being read |
