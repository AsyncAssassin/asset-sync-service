...
- HTTP Basic reads the user store through a cache. A user loaded in the last minute comes from memory, and during the outage a user loaded in the last ten minutes stays accepted, with the database asked again once a minute by one request while the others keep the cached user. So `/actuator/metrics` and `/actuator/prometheus` keep answering for those users; the outbox gauges show their last background refresh. A user the cache does not hold waits for the connection timeout and gets `503`, as does any request whose own handling needs the database. Anonymous health probes need no credentials.
