...
- `demo` runs only with the HTTP bridge. `ASSET_SYNC_PROVIDER_TYPE=alchemy` stops the start: the seeded `local-evm` address would fail the Alchemy preflight on every start after the first. Run Alchemy under `prod`.
...
- GitHub Actions CI runs Gradle checks, `bootJar`, a Trivy scan of the Docker image that fails on HIGH and CRITICAL vulnerabilities with a fix, `docker compose config`, and a generated jOOQ tracking guard. The scan runs on every push, so a newly published fixable CVE turns `main` red even on a commit that changes only docs: check the badge before a demo. Dependabot does not update the Trivy image, which the workflow runs pinned by digest; bump it by hand. The workflow token is read-only, actions are pinned to commit SHAs, and the Gradle wrapper verifies the distribution checksum.
...
- `REVERTED` is final. A transaction that a reorg reverted stays `REVERTED` even if the chain includes it again; a later event for it is a duplicate or a conflict. Nothing tracks reorgs automatically: `REVERTED` arrives through `POST /api/v1/observed-events` or from an HTTP bridge that reports it in an event, while the Alchemy adapter never reports it and scans only up to the finality frontier, where reorgs are practically excluded.
...
- Users are cached for a minute, so an old password, or a user removed directly in the `users` table or on another instance, keeps working for up to a minute; a new password works at once. During a database outage a user seen in the last ten minutes stays accepted, which keeps the metrics reachable, but one request per user and minute waits for the connection timeout while it reads the database again.
...
- Outbox retention removes only `PUBLISHED` rows. `DEAD` rows, the deliveries that ran out of attempts, stay until an operator deletes them; `docs/database.md` shows how.
...
- Transfers with a zero amount are recorded like any other; the Alchemy adapter asks for them (`excludeZeroValue=false`). Zero-value spam sent to a watched address, as in address poisoning, reaches the outbox, so a consumer that shows transfers to people should filter it.
