...
- The `demo` provider base URL follows `server.port` instead of the `SERVER_PORT` variable, so the self-call into the simulator works however the port is set, including `--server.port` and IDE run configurations.
- The `demo` seeder writes each outbox row as a complete lifecycle event of its transaction: the full payload, an event type that matches the transaction's status, and an idempotency key in the service's format. Published seed events no longer log `null` fields. Seeded transactions and events record the source `demo:seed`. A database seeded by an earlier version keeps its rows; start from an empty one (`docker compose down -v`) to get the current dataset.
