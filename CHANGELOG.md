...
- **Breaking:** a control character inside `externalRef`, `label`, `chainId`, `asset`, or an ingested `address`, U+0000 to U+001F and U+007F to U+009F, fails validation with a `400 validation-failed` that names the field. Surrounding whitespace is still trimmed, and `externalRef` and `label` may still hold tabs and line breaks. A NUL byte used to reach PostgreSQL, which cannot store it, and came back as `400 database-constraint-violation` with a WARN line; other control characters were stored.
...
- A bridge page whose cursor or checkpoint metadata PostgreSQL cannot store, because of a NUL character or metadata over `max-checkpoint-json-length` as PostgreSQL writes it out, fails before its events are written. It failed at the checkpoint write after them, as a database error, and on every later sync again.
...
- An address disabled through the API while it syncs, or whose chain is disabled meanwhile, no longer fails as an address error. A disabled address's next page failed with `Provider returned an event for an address that is not watched`, a disabled chain's with `Chain <chainId> is not enabled, so its events cannot be ingested`, and an account sync recorded either as an address failure and ended `FAILED`. Such an address now stops at the page being fetched: its own run fails with `Watched address was disabled during the sync.` or `Chain <chainId> is not enabled, so its addresses are not synced.`, and an account pass leaves it out without an error. An active address the provider's events do not match stays an error of that address.
...
- A number with a fraction in an integer field fails the request with `400 invalid-request`. Jackson dropped the fraction, so an `eventIndex` of `1.9` was stored as `1`. A whole number written as a float, such as `1.0` or `1e3`, is still read as that integer. A bridge page with a fraction in an integer field is now bad data of that address, and the error names the field.
...
[Unreleased]: https://github.com/AsyncAssassin/asset-sync-service/compare/v0.4.2...HEAD
[0.4.2]: https://github.com/AsyncAssassin/asset-sync-service/compare/v0.4.1...v0.4.2
