...
- Syncs skip an address whose chain is disabled, as they skip a disabled address: an account sync leaves it out, and `POST /api/v1/addresses/{addressId}/sync` answers `404 Unsupported chain`. Such an address failed every account sync it belonged to, so disabling a chain was not enough to take its addresses out.
...
- One address whose provider fetch keeps failing retryably, such as a timeout on a heavy address or a `5xx`, no longer blocks its account. An account sync failed the whole claim on it and began the next claim at that address again, so the run failed after five attempts and the addresses after it were never synced; each failed claim also lost the progress of the pass. Such an address is now deferred and retried once in each later claim, `retry-backoff-base-delay` apart (`last_requeue_reason = ADDRESS_RETRY`, changeset 018, no `failure_attempts` spent), and recorded as failed after three failed retries while the others sync. Three addresses failing in a row, or two in a claim in which the provider served nothing, still fail the claim as a provider outage, and a failed or interrupted claim now saves the pass, so the retried run resumes where it stopped.
